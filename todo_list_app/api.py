...
from todo_list_app.crud import (
    create_task_list_db,
    get_task_list_by_id_db,
    create_task_db,
    update_task_list_db,
...
def get_task_list(task_list_id: int, scope):
    ...
    if user_role == 'admin':
        ...
        if task_list.user_id == user_id:
            ...
            return {
                'error': 'You do not have permission to access this task list.'
            }
...
def create_task_list(task_list: CreateTaskList, scope):
    ...
    return {
        ...
        'task_list_name': new_task_list.name,
    }
...
def create_task(task: CreateTask, scope):
    ...
    with get_session() as session:
        task_list = (
            session.query(TaskList).filter(TaskList.id == task.list_id).first()
        )
...
def update_task_list(updated_task_list: UpdateTaskList, scope):
    ...
    with get_session() as session:
        task_list = (
            ...
            .filter(TaskList.id == updated_task_list.list_id)
            .first()
        )
        ...
        if task_list.user_id != user_id:
            return {
                'error': 'You do not have permission to update this task list.'
            }

    updated_task_list = update_task_list_db(
        updated_task_list.list_id, updated_task_list.name
    )
...
def delete_task_list(list_id: int, scope):
    ...
    if task_list.user_id != user_id:
        return {
            'error': 'You do not have permission to delete this task list.'
        }
    ...
    return {
        'message': f'Task list "{deleted_task_list_name}" deleted '
        f'successfully.'
    }
